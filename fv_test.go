...
func TestEmbed(t *testing.T) {
	...
	if foo1.Distance(foo2.F) != 0 {
		t.Error("Expected distance 0")
	}
}

func TestTriangulage(t *testing.T) {
	testTrig(F{2, 3}, 2, 1, 2, 3, t)
	testTrig(F{1, 4}, 1.23, 0, math.Pi, 3, t)

}

func testTrig(p F, m, a1, a2, a3 float64, t *testing.T) {
	p1 := p.Add(P{m, a1}.F())
	p2 := p.Add(P{m, a2}.F())
	p3 := p.Add(P{m, a3}.F())
...
func TestLineIntersect(t *testing.T) {
	// normal line
	l1 := F{0, 1}.LineTo(F{1, 2})
	l2 := F{1, 0}.LineTo(F{2, 3})
	testLine(l1, l2, t)

	// l1 is vertical
	l1 = F{0, 1}.LineTo(F{0, 2})
	l2 = F{1, 0}.LineTo(F{2, 3})
	testLine(l1, l2, t)

	// l2 is vertical
	l1 = F{0, 1}.LineTo(F{1, 2})
	l2 = F{1, 0}.LineTo(F{1, 3})
	testLine(l1, l2, t)

	// lines are parallel
	l1 = F{0, 1}.LineTo(F{1, 2})
	l2 = F{1, 2}.LineTo(F{2, 3})
